...
plot(ax1,p,T.P_accept,'-o','Color',dark,'MarkerFaceColor',dark,'MarkerSize',3.5,'LineWidth',1.4);
plot(ax1,p,T.P_reject,'--s','Color',red,'MarkerFaceColor',red,'MarkerSize',3.5,'LineWidth',1.2);
xline(ax1,main.p0,':','Color',ink,'LineWidth',1); xline(ax1,main.p1,':','Color',ink,'LineWidth',1);
yline(ax1,.95,':','Color',[.35 .35 .35]); yline(ax1,.10,':','Color',[.35 .35 .35]);
...
legend(ax1,{'P_{accept}(p)','P_{reject}(p)','p_0=0.10','p_1=0.13'},'Location','southwest','FontName',fontNameCN(),'Box','off','TextColor',ink,'Color','w');
...
sgtitle(fig,sprintf('问题一主情景的 OC 与 ASN（状态：%s）',string(S.status)), ...
