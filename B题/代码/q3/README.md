...
程序实现 Q3-M3 装配树—吸收 Markov 奖励模型，完整枚举 Q3-A1 的 65536 个固定策略。位 0--7 是零件 `x1..x8`，位 8--10 是半成品 `y1..y3`，位 11 是 `yf`，位 12--14 是 `z1..z3`，位 15 是 `zf`。
...
- 所有局部核传播谱半径、吸收裕度、条件数和残差。裕度不大于 `1e-10` 时标记 `NEAR_NONABSORBING` 并高精度复核；残差超限不得返回 `SUCCESS_EXACT`。
